...
async function performLcr(sequelize, logger, calledNumber) {

  const r = await sequelize.query(sqlRegex, {
    //logging: logger,
    plain: false,
    raw: true,
    replacements: [calledNumber],
  ...
  if (r.length === 0) {
    ...
    return randomizeOutboundGateways(sequelize, logger, calledNumber);
  }
  ...
  const carriers = await sequelize.query(sqlCarrierSet, {
    //logging: logger,
    plain: false,
    raw: true,
    replacements: [lcr_route.lcr_route_sid],
    type: QueryTypes.SELECT
  });
  ...
  const r2 = await sequelize.query(sqlGateways, {
    //logging: logger,
    plain: false,
    raw: true,
    replacements: [vcsids],
...
async function randomizeOutboundGateways(sequelize, logger, calledNumber) {
  const r = await sequelize.query(sqlGatewaysAll, {
    type: QueryTypes.SELECT
  });
